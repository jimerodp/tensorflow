...
struct XprofDomain {
  static const char* name() { return "xprof"; }
};

// Note, static DOMAIN::name() must return a valid c string constant.
template <typename DOMAIN>
nvtxDomainHandle_t NvtxDomainOf() {
  static nvtxDomainHandle_t domain = nvtxDomainCreateA(DOMAIN::name());
  return domain;
}

template <typename DOMAIN>
nvtxStringHandle_t RegisteredMessage(const char* message) {
  return nvtxDomainRegisterStringA(NvtxDomainOf<DOMAIN>(), message);
}
...
class NvtxScopedRange final {
 public:
  explicit NvtxScopedRange(const char* range_name) {
    ...
    event_attr.message.registered = RegisteredMessage<DOMAIN>(range_name);
    nvtxDomainRangePushEx(NvtxDomainOf<DOMAIN>(), &event_attr);
  }

  ~NvtxScopedRange() { nvtxDomainRangePop(NvtxDomainOf<DOMAIN>()); }
};
...
#define SCOPEDRANGE(N) NvtxScopedRange<XprofDomain> range##__LINE__(N)
